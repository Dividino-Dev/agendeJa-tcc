...
export function AppointmentList() {
  ...
  const handleDelete = async () => {
    ...
    try {
      setIsDeleting(true)
      const res = await api.delete(`/schedulings/${schedulingToDelete.id}`)
      if (res.status === 200) {
            ...
            {futureSchedulings.map((scheduling) => (
              <div key={scheduling.id} className="flex items-center justify-between p-4 bg-white rounded-lg border">
                <div className="flex items-center gap-4">
                  <Avatar>
                    <AvatarImage
                      src={`${process.env.NEXT_PUBLIC_API_URL}/static/uploads/${scheduling.professional.userId}.png`}
                      alt={scheduling.professional.user.name}
                  ...
                  <div>
                    <h3 className="font-medium">{scheduling.professional.user.name}</h3>
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              <span className="font-medium">{schedulingToDelete?.professional.user.name}</span> para{" "}
          ...
          <DialogFooter className="flex flex-row justify-end gap-2 sm:justify-end">
            ...
            <Button variant="destructive" onClick={handleDelete} disabled={isDeleting}>
              {isDeleting ? "Excluindo..." : "Sim, excluir agendamento"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
